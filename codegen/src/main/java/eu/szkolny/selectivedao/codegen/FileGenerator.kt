...
package eu.szkolny.selectivedao.codegen
...
class FileGenerator : AbstractProcessor() {
    ...
    private fun processClass(dao: TypeElement, file: File) {
        ...
        usedTypeConverters.forEach { converter ->
            selective.addProperty(
                PropertySpec.builder("__${converter.converterType.simpleName}", converter.converterType.asType().asTypeName(), KModifier.PRIVATE)
                .delegate(
                    CodeBlock.builder()
    ...
    private fun processMethod(cls: TypeSpec.Builder, method: ExecutableElement, annotation: UpdateSelective, typeConverters: List<TypeConverter>): List<TypeConverter> {
        ...
        cls.addProperty(
            PropertySpec.builder(adapterName, entityInsertionAdapter.parameterizedBy(paramTypeElement.asClassName()), KModifier.PRIVATE)
